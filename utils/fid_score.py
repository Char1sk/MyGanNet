...
def compute_statistics_of_path(path:Union[str, List[str], Tuple[str]],
                               model, batch_size, dims, device, num_workers=1) -> float:
    # print(type(path))
...
def calculate_fid_given_paths(path1:Union[str, List[str], Tuple[str]], path2:Union[str, List[str], Tuple[str]],
                              batch_size, device, dims, num_workers=1, path='../Models/pt_inception.pth') -> float:
    """Calculates the FID of two paths"""
    # print(paths)
    if isinstance(path1, str) and not os.path.exists(path1):
        raise RuntimeError('Invalid path: %s' % path1)
    if isinstance(path2, str) and not os.path.exists(path2):
...
def get_fid(path1:Union[str, List[str], Tuple[str]], path2:Union[str, List[str], Tuple[str]],
            device=None, num_workers=None, path='../Models/pt_inception.pth') -> float:
...
def get_folders_from_list(folder:str, file:str) -> List[str]:
    '''get a List of folders from a folder-list'''
