...
def partition_image(img:Tensor, h_ratio:float, w_ratio:float, architecture:str='SE', all_D:bool=False, final_parts:bool=False) -> Tuple[Tensor]:
    # only works for C*H*W or B*C*H*W tensors
    h, w = img.shape[-2:]
    ht, wl = int(h*h_ratio), int(w*w_ratio)
    if architecture == 'SE' or all_D or final_parts:
        if img.dim() == 3:
            img_tl = img[:, :ht, :wl]
            img_tr = img[:, :ht, wl:]
            img_d = img[:, ht:, :]
